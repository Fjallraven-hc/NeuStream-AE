...
from models import DiT_models, DiT_S_2
import argparse
...
class DiT_pipe:
    def __init__(self, device):
        self.device = device
        self.latent_size = 32
        self.DiT = DiT_S_2(input_size=32, num_classes=1000).to(device).half()
        vae_path = "../sd-vae-ft-mse"
        # self.vae = AutoencoderKL.from_pretrained(f"stabilityai/sd-vae-ft-mse").to(device).half()
        self.vae = AutoencoderKL.from_pretrained(vae_path).to(device).half()
