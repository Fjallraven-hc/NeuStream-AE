...
#class Worker(threading.Thread):
    def __init__(self, stream_module_list, input_queue, output_queue, id, log_prefix, deploy_ready, extra_vae_safety_time, image_size, profile_device, step_slo_scale: float, step_delta: float, **kwargs):
        ...
        if profile_device == "h100":
            ...
            if image_size == 256:
                self.profile_latency = self.rtx4090_profile_latency_256_2024
                self.batch_upper_bound = 40
            elif image_size == 512:
    ...
    def gather(self, **kwargs):
        ...
        while True:
            ...
            while not self.input_queue.empty():
                ...
                if self.id == "clip":
                    ...
                    if request["SLO"] + request["request_time"] < time.time() + self.profile_latency["vae"]["1"] + self.profile_latency["safety"]["1"]:
                        ...
                        #print(f'right value: {time.time() + self.profile_latency["vae"]["1"] + self.profile_latency["safety"]["1"]}')
                        info = f"pid: [{os.getpid()}], holding module: {self.module_tag}, abandon one request. id:{request['id']}"
                        self.log_file.write(info+"\n")
                        print(info)
                        #print(f"pid: [{os.getpid()}], holding module: {self.module_tag}, abandon one request.")
            ...
            # 在stable diffusion的case里，只有unet需要这样做，clip或者vae都会早早扔掉
            if self.id == "unet":
                valid_request_list = []
        ...
        with torch.inference_mode():
            try:
                ...
                while True:
                    ...
                    if self.id == "unet":
                        # instantly profile batch latency
                    ...
                    if self.id == "unet":
                        profile_latency = time.perf_counter() - begin
