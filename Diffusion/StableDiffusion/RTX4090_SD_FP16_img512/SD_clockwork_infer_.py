...
if args.image_size == 256:
    for steps in range(30, 51):
        ...
        for batch_size in range(1, 16):
            pro[steps][batch_size] = module_latency_rtx4090_256["clip"][str(batch_size)] + module_latency_rtx4090_256["unet"][str(batch_size)] * steps + module_latency_rtx4090_256["vae"][str(batch_size)] + module_latency_rtx4090_256["safety"][str(batch_size)]
elif args.image_size == 512:
    for steps in range(30, 51):
        pro[steps] = {}
        for batch_size in range(1, 16):
            pro[steps][batch_size] = module_latency_rtx4090_512["clip"][str(batch_size)] + module_latency_rtx4090_512["unet"][str(batch_size)] * steps + module_latency_rtx4090_512["vae"][str(batch_size)] + module_latency_rtx4090_512["safety"][str(batch_size)]
...
if __name__ == "__main__":
    ...
    RTX4090_SD_FP16_img512_trace = json.load(open("RTX4090_SD_FP16_img512_trace.json"))
    ...
    for idx in range(num_worker):
        pipes.append(StableDiffusionPipeline.from_pretrained(model_path, torch_dtype=torch.float16).to(f"cuda:2"))
    # """pipes = [        
