...
class QKVAttentionLegacy(nn.Module):
    ...
    def forward(self, qkv):
        ...
        weight = torch.softmax(weight.half(), dim=-1).type(weight.dtype)
...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        self.use_checkpoint = use_checkpoint
        self.dtype = torch.float32 if use_fp16 else torch.float32
        self.num_heads = num_heads
    ...
    def forward(self, x, gammas):
        ...
        # print(f'yhc debug: gammas.dtype={gammas.dtype}')
        emb = self.cond_embed(gamma_embedding(gammas, self.inner_channel))

        h = x.type(torch.float16)
        # print(f'yhc debug:: emb.dtype={emb.dtype}')
        # print(f'yhc debug:: h.dtype={h.dtype}')
