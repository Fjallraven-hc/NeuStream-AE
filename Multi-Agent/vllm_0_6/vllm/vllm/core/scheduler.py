...
def read_params(model_type: str):
    try:
        p = str(Path(__file__).parent.parent.parent.parent.parent.resolve()) # / "profile_data" / model_type + ".txt"
        p = f"{p}/profile_data/{model_type}.txt"
        with open(p, "r") as f:
            lines = f.readlines()
    except:
...
class Scheduler:
    ...
    def __init__(
        ...
        # print(f"p_time: {p_time}, p_para: {p_para}, d_para: {d_para}")
        try:
    ...
    def _schedule_default(self) -> SchedulerOutputs:
        ...
        if do_prefill:
            if self.do_predict and num_prefill >= 0:
        ...
        if len(prefills.seq_groups) == 0:
            ...
            # print("d", len(running_scheduled.decode_seq_groups))
        else:
            # print("p", len(prefills.seq_groups))
    ...
    def schedule(
        ...
        scheduler_outputs, pred_time = self._schedule()
        now = time.time()
        ...
        return (seq_group_metadata_list, scheduler_outputs,
                allow_async_output_proc, pred_time)
