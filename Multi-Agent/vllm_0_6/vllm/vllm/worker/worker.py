...
class Worker(LocalOrDistributedWorkerBase):
    ...
    def determine_num_available_blocks(self) -> Tuple[int, int]:
        ...
        print(f"used memory: {used_memory // 2**20} MB")
        ...
        used_memory_ = torch.cuda.memory_reserved()
        print(f"Used memory after: {used_memory_ // 2**20} MB, diff: {(used_memory_ - used_memory) // 2**20} MB")
        diff = used_memory_ - used_memory
        ...
        print(f"memory overhead: {2 * diff / (used_memory_ + num_gpu_blocks * cache_block_size) * 100:.2f}%") # llava
