...
def frontend(input_queue: mp.Queue,output_queue: list[mp.Queue], final_output: mp.Queue):
    logger = logging.getLogger("frontend")
    logger.setLevel(logging.DEBUG)
    add_consolehandler(logger)
    # add_filehandler(logger, "/home/xwh/new_vllm_docker/MatPlotAgent/log/frontend.log")
    add_filehandler(logger, "./log/frontend.log")
    ...
    for ar_,cv_,slo_,pred in test: 
        ...
        shape = 1 / cv_**2
        scale = cv_**2 / ar_
        interval = list(np.random.gamma(shape, scale, 300))
...
def finalend(final_output: mp.Queue):
    ...
    logger.setLevel(logging.DEBUG)
    add_consolehandler(logger)
    add_filehandler(logger, "./log/final.log")
    while True:
        item = final_output.get()
        # if item is None:
        #     break
        req_id = item["req_id"]
...
if __name__ == '__main__':
    ...
    p_frontend = mp.Process(target=frontend, args=(frontend_queue,[codellama_1,code_executor_1,llava_1,llava_2,codellama_2,code_executor_2], final_ouput))
