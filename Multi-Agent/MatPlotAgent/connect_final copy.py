...
def frontend(input_queue: mp.Queue,output_queue: list[mp.Queue]):
    ...
    for ar_,cv_,slo_,pred in test: 
        ...
        while warm_model:
            ...
            address, _, item = socket.recv_multipart()
            if item not in warm_model:
                raise
            warm_model.remove(item)
            address_list.append(address)
        for address in address_list:
            socket.send_multipart([address, b'', str((slo_,pred)).encode()])
...
if __name__ == '__main__':
    ...
    codellama_1 -> code_executor_1
    codellama_1 -> llava_1
    codellama_1 -> llava_2
    llava_1 -> codellama_2
    llava_2 -> codellama_2
    codellama_2 -> code_executor_2
    code_executor_2 -> final_output
    final_output -> frontend,

    in the using of multiprocessing queuq i can have the following,
    ...
    I want to know in the networking env how to simulate the above configuration conveniently.
    """
