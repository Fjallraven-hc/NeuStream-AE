...
"""
zeroMQ: 
    Note network topological structure.
"""
...
cv_seed = {1: 4, 2: 6, 3: 7, 4: 6}
DUMP = 3
# TODO: solve cv exp
ori_test = [
    # [0.6, 1, 1.8], [0.7, 1, 1.8], [0.8, 1, 1.8], [0.9, 1, 1.8], [1.0, 1, 1.8],
    # (0.6, 1, 1.8), (0.7, 1, 1.8), (0.8, 1, 1.8), (0.9, 1, 1.8), (1.0, 1, 1.8),
    # (0.8, 1, 1.8), (0.8, 2, 1.8), (0.8, 3, 1.8), (0.8, 4, 1.8),
...
port_map = {
    ...
    2: {
        "src": 5558, # connect pull
        "dst": 5560 # excep: connect dealer
    },
    3: {
        "src": 5558, # connect dealer
        "dst": 5561 # excep: connect dealer
    },
    4: {
        "src": [5560, 5561], # bind ROUTER
        "dst": 5562 # bind
    },
    5: {
        "src": 5562, # connect
...
def frontend(input_queue: mp.Queue,output_queue: list[mp.Queue]):
    ...
    all_port = port_map[-1]
    socket.bind(f'tcp://*:{all_port}')

    ## To codellama_1
    context_2 = zmq.Context()
    socket_2 = context_2.socket(zmq.PUSH)
    socket_2.bind(f'tcp://*:{port_map[0]["src"]}')
    ...
    for ar_,cv_,slo_,pred in test: 
        ...
        # logger.debug('Frontend send data!')
        print(address_list)
        for address in address_list.values():
            socket.send_multipart([address, pickle.dumps((slo_,pred))])
        logger.debug('Frontend send data finish!')
        warm_model = gpu_model # [:1]
        assert warm_model[0] == 0
        while warm_model:
            address, item = socket.recv_multipart()
            item = pickle.loads(item)
            print(item)
            if item not in warm_model:
                raise
        ...
        for i in range(loop):
            ...
            socket_2.send(pickled_data)
        # output_queue[0].put(None)
        socket_2.send(pickle.dumps(None))

def finalend(final_output: mp.Queue):
    logger = logging.getLogger("final")
    logger.setLevel(logging.DEBUG)
    add_consolehandler(logger)
    add_filehandler(logger, "/home/xwh/new_vllm_docker/MatPlotAgent/log/final.log")
    context = zmq.Context()
    socket = context.socket(zmq.PULL)
    socket.connect(f'tcp://localhost:{port_map[5]["dst"]}')
    logger.debug("FINAL OK!")
    while True:
        data = socket.recv()
        ...
        if item is None:
            break
        req_id = item["req_id"]
        arrival_time = item["org_arrival_time"]
        real_time = item["real_time"]
        logger.debug(f"Request id: {req_id}, e2e: {time.time()-arrival_time}, real_time: {real_time}")
...
def model_running(input_queue: mp.Queue, output_queue: mp.Queue, model_id: int, config, frontend_queue: mp.Queue=None):
    ...
    ## OK
    src = port_map[model_id]["src"]
    dst = port_map[model_id]["dst"]
    all_port = port_map[-1]
    socket_1 = context_1.socket(zmq.PULL)
    if model_id != 4:
        socket_1.connect(f"tcp://localhost:{src}")
    else:
        for s in src:
            socket_1.connect(f"tcp://localhost:{s}")

    socket_2 = context_2.socket(zmq.PUSH)
    socket_2.bind(f'tcp://*:{dst}')
    
    ## ALWAYS correct
    socket_3 = context_3.socket(zmq.DEALER)
    socket_3.connect(f'tcp://localhost:{all_port}')
    ...
    llm, other_params = get_model(socket_1, socket_2, model_id, config)
    ...
    for index in range(0,test_num):
        ...
        socket_3.send(pickle.dumps(model_id))
        if model_id in GPU_MAP.keys():
        ...
        outputs = llm.serve(socket_1, socket_2, **other_params)
...
if __name__ == '__main__':
    ...
    p_final = mp.Process(target=finalend, args=(final_ouput,))

    process_ls.extend([p_frontend, p_codellama_1,p_code_executor_1, p_llava_1,p_llava_2, p_codellama_2, p_code_executor_2, p_final]) 
    # process_ls.extend([p_frontend, p_codellama_1])
    for p in process_ls:
        p.start()
    for p in process_ls:
        p.join()
